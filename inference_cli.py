...
from src.utils.model_registry import DEFAULT_DIT, DEFAULT_VAE, get_available_dit_models
...
def _parse_offload_device(
    offload_arg: str, platform_type: str = None, cache_enabled: bool = False
) -> Optional[str]:
    """
...
def extract_frames_from_image(image_path: str) -> Tuple[torch.Tensor, float]:
    ...
    if frame.shape[2] == 4:
        ...
        debug.log("Detected RGBA image (alpha channel preserved)", category="file")
    else:
    ...
    debug.log(
        f"Image tensor shape: {frames_tensor.shape}, dtype: {frames_tensor.dtype}",
        category="memory",
    )
...
def get_input_type(
    input_path: str,
) -> Literal["video", "image", "directory", "unknown"]:
    """
...
def process_single_file(
    ...
    format_auto_detected: bool = False,
    ...
    debug.log(
        f"Processing {input_type}: {Path(input_path).name}",
        category="generation",
        force=True,
    )
    ...
    if input_type == "video":
        ...
        debug.log(
            f"Frame extraction time: {time.time() - start_time:.2f}s", category="timing"
        )
    else:
    ...
    if output_path is None:
        output_path = generate_output_path(
            input_path, args.output_format, input_type=input_type
        )
    elif not Path(output_path).suffix or (
        args.output_format == "png" and input_type != "image"
    ):
    ...
    debug.log(
        f"{format_prefix} output format: {args.output_format}",
        category="info",
        force=True,
        indent_level=1,
    )
    ...
    debug.log(
        f"Processing time: {time.time() - processing_start:.2f}s", category="timing"
    )
    ...
    if is_png_format and not is_single_image:
        debug.log(
            f"PNG frames saved in directory: {output_path}", category="file", force=True
        )
    else:
...
def extract_frames_from_video(
    video_path: str, skip_first_frames: int = 0, load_cap: Optional[int] = None
    ...
    debug.log(
        f"Video info: {frame_count} frames, {width}x{height}, {fps:.2f} FPS",
        category="info",
    )
    ...
    while True:
        ...
        if debug.enabled and frames_loaded % 100 == 0:
            ...
            debug.log(
                f"Extracted {frames_loaded}/{total_to_load} frames", category="file"
            )
...
def save_frames_to_video(
    frames_tensor: torch.Tensor, output_path: str, fps: float = 30.0
) -> None:
    """
    ...
    """
    debug.log(
        f"Saving {frames_tensor.shape[0]} frames to video: {output_path}",
        category="file",
    )
...
def save_frames_to_png(
    frames_tensor: torch.Tensor, output_dir: str, base_name: str
) -> None:
    """
    ...
    """
    debug.log(
        f"Saving {frames_tensor.shape[0]} frames as PNGs to directory: {output_dir}",
        category="file",
    )
    ...
    debug.log(
        f"PNG saving completed: {total} files in '{output_dir}'", category="success"
    )
...
def _process_frames_core(
    ...
    dit_offload = _parse_offload_device(
        args.dit_offload_device, platform_type, cache_dit
    )
    vae_offload = _parse_offload_device(
        args.vae_offload_device, platform_type, cache_vae
    )
    tensor_offload = _parse_offload_device(
        args.tensor_offload_device, platform_type, False
    )
    ...
    ctx = decode_all_batches(
        runner, ctx=ctx, debug=debug, progress_callback=None, cache_model=cache_vae
    )
...
def _gpu_processing(
    frames_tensor: torch.Tensor, device_list: List[str], args: argparse.Namespace
) -> torch.Tensor:
    """
    ...
    if args.temporal_overlap > 0 and num_devices > 1:
        ...
        if args.batch_size > 1:
            chunk_with_overlap = (
                (chunk_with_overlap + args.batch_size - 1) // args.batch_size
            ) * args.batch_size
    ...
    if args.temporal_overlap > 0 and num_devices > 1:
        ...
        for idx, res_np in enumerate(results_np):
            ...
            if idx == 0:
                ...
                if (
                    chunk_tensor.shape[0] > overlap
                    and result_tensor.shape[0] >= overlap
                ):
                    # Get overlapping regions
                    prev_tail = result_tensor[
                        -overlap:
                    ]  # Last N frames from accumulated result
                    cur_head = chunk_tensor[
                        :overlap
                    ]  # First N frames from current chunk
                    ...
                    result_tensor = torch.cat(
                        [
                            ...
                            chunk_tensor[
                                overlap:
                            ],  # Non-overlapping part of current chunk
                        ],
                        dim=0,
                    )
                else:
                    ...
                    if chunk_tensor.shape[0] > overlap:
                        result_tensor = torch.cat(
                            [result_tensor, chunk_tensor[overlap:]], dim=0
                        )
        ...
        result_tensor = torch.from_numpy(np.concatenate(results_np, axis=0)).to(
            torch.float32
        )
    ...
    if args.prepend_frames > 0:
        if args.prepend_frames < result_tensor.shape[0]:
            debug.log(
                f"Removing {args.prepend_frames} prepended frames from output",
                category="generation",
            )
            result_tensor = result_tensor[args.prepend_frames :]
        else:
            debug.log(
                f"prepend_frames ({args.prepend_frames}) >= total frames ({result_tensor.shape[0]}), skipping removal",
                level="WARNING",
                category="generation",
                force=True,
            )
...
"""
    ...
    # Auto-show help if no arguments provided for CLI usage.
    # When called programmatically (argv is not None) we never mutate sys.argv.
...
def main(cli_args=None) -> None:
    ...
    if isinstance(cli_args, argparse.Namespace):
        # Already parsed Namespace provided programmatically.
        args = cli_args
    elif isinstance(cli_args, list):
        # Raw CLI-style argv list provided programmatically.
        args = parse_arguments(cli_args)
    else:
        # Standard CLI invocation using sys.argv.
    ...
    if (
        args.vae_encode_tiled
        and args.vae_encode_tile_overlap >= args.vae_encode_tile_size
    ):
        debug.log(
            f"VAE encode tile overlap ({args.vae_encode_tile_overlap}) must be smaller than tile size ({args.vae_encode_tile_size})",
            level="ERROR",
            category="vae",
            force=True,
        )
    ...
    if (
        args.vae_decode_tiled
        and args.vae_decode_tile_overlap >= args.vae_decode_tile_size
    ):
        debug.log(
            f"VAE decode tile overlap ({args.vae_decode_tile_overlap}) must be smaller than tile size ({args.vae_decode_tile_size})",
            level="ERROR",
            category="vae",
            force=True,
        )
    ...
    if args.cache_dit and args.dit_offload_device == "none":
        offload_target = (
            "system memory (CPU)" if _get_platform_type() != "mps" else "unified memory"
        )
        debug.log(
    ...
    if args.debug:
        if platform.system() == "Darwin":
            debug.log(
                "You are running on macOS and will use the MPS backend!",
                category="info",
                force=True,
            )
        else:
            ...
            debug.log(
                f"CUDA_VISIBLE_DEVICES: {os.environ.get('CUDA_VISIBLE_DEVICES', 'Not set (all)')}",
                category="device",
            )
            if torch.cuda.is_available():
                ...
                debug.log(
                    "Using device index 0 inside script (mapped to selected GPU)",
                    category="device",
                )
    ...
    try:
        ...
        if platform.system() == "Darwin":
            ...
            if args.cuda_device:
                device_list = [
                    d.strip() for d in str(args.cuda_device).split(",") if d.strip()
                ]
            else:
                device_list = ["0"]
        if args.debug:
        ...
        if not download_weight(
            ...
            debug.log(
                "Failed to download required models. Check console output above.",
                level="ERROR",
                category="download",
                force=True,
            )
        ...
        if input_type == "directory":
            ...
            if not media_files:
                debug.log(
                    f"No video or image files found in directory: {args.input}",
                    level="ERROR",
                    category="file",
                    force=True,
                )
            ...
            debug.log(
                f"Found {len(media_files)} media files to process",
                category="file",
                force=True,
            )
            ...
            for idx, file_path in enumerate(media_files, 1):
                ...
                debug.log(
                    f"Processing file {idx}/{len(media_files)}",
                    category="generation",
                    force=True,
                )
            ...
            debug.log(
                f"Unsupported input type: {args.input}",
                level="ERROR",
                category="file",
                force=True,
            )
        ...
        debug.log("", category="none", force=True)
        debug.log(
            f"All upscaling processes completed successfully in {total_time:.2f}s",
            category="success",
            force=True,
        )
        ...
        if total_time > 0 and total_frames_processed > 0:
            ...
            debug.log(
                f"Average FPS: {fps:.2f} frames/sec", category="timing", force=True
            )
        ...
        debug.log(
            f"Process {os.getpid()} terminating - VRAM will be automatically freed",
            category="cleanup",
            force=True,
        )
